...
{	
  ...
  //LineList keyValueArray;  
      ...
      //unsigned int wid = trie->findPartition( buff );
      //pc[ wid ]->push_back( buff );
	keyValue[inputId].push_back(buff);      
    }
		inputFile.close();
...
/* for(int i = 0; i < conf->getNumInput(); i++){
	PartitionCollection& pc = inputPartitionCollection[inputId];
} */

 // Crate lists of lines
/*  for ( unsigned int i = 0; i < conf->getNumReducer(); i++ ) {
    pc[ i ] = new LineList;//cout<<i<<"\n";
    // inputPartitionCollection[ inputId ][ i ] = new LineList;
  }*/
	...
	//cout<<rank<<":";
for(unsigned int i = 0; i < conf->getNumInput(); i++){
	PartitionCollection& pc = inputPartitionCollection[i];
	for(unsigned int k = 0; k < conf->getNumReducer(); k++){
		pc[i][k] = new LineList;
		for ( unsigned long j = 0; j < keyValue[i].size(); j++ ) {
      			unsigned int wid = trie->findPartition( keyValue[i][j] );
      			pc[i][ wid ]->push_back( keyValue[i][j] );
			//cout<<wid<<" ";
		}
	}
}
