...
{	
  ...
  LineList keyValue[conf->getNumInput()];
  ...
  // Partition data in the input file  
	for ( unsigned long i = 0; i < numLine; i++ ) {
      ...
      keyValue[inputId-1].push_back( buff );	
    }
//	keyValue[inputId-1].push_back(keyValueArray);
	//cout<<rank<<" : "<<inputId<<"\n";
	inputFile.close();
 }
  ...
  for(unsigned int f = 0; f < conf->getNumInput(); f++ ){
   if(inputSet.find(f+1) != inputSet.end()){
    PartitionCollection& pc = inputPartitionCollection[ f+1 ];
    //cout<<rank<<" : "<<keyValue[f].size()<<"\n";
    for ( unsigned int i = 0; i < conf->getNumReducer(); i++ ) {
        pc[ i ] = new LineList;
        }
	for(unsigned long j = 0; j < keyValue[ f ].size(); j++){
            //unsigned int* buff = keyValue[f][j];
	    unsigned int wid = trie->findPartition( keyValue[f][j] );	   
	    if( wid != conf->getNumReducer()-1 ){
                pc[ wid ]->push_back( keyValue[f][j] );
            }
            else{
		//cout<<wid<<" ";
                if( cmpKey(keyValue[f][j], maxKey, conf->getKeySize()) ){
                    pc[ wid ]->push_back( keyValue[f][j] );
	
                }   
            }   
        }
//	cout<<rank<<" : "<<pc[0]->size()<<" "<<pc[1]->size()<<"\n";
     NodeSet fsIndex = cg->getNodeSetFromFileID( f+1 );
       for ( unsigned int i = 0; i < conf->getNumReducer(); i++ ) {
    ...
    //inputFile.close();
	  time += clock();
	  rTime = double( time ) / CLOCKS_PER_SEC; //cout<<rTime<<endl;
	  MPI::COMM_WORLD.Gather( &rTime, 1, MPI::DOUBLE, NULL, 1, MPI::DOUBLE, 0 ); 
...
/* void CodedWorker::execReduce(LineList array[])
{
...
//ADDED BY ME
LineList CodedWorker::merge(LineList in1, LineList in2){
  LineList sorted;
...
LineList CodedWorker::SortSorted(LineList array[]){
    LineList temp;
